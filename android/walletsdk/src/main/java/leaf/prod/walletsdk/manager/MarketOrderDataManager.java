...
//		}
	...
	private Double calculateGas(String symbol, Double amount, Double lrcFee) {
		Double result;
		AccountBalance asset = balance.getAssetBySymbol(symbol);
		...
		if (symbol.equalsIgnoreCase("LRC")) {
			...
			Double tokenFrozen = balance.getAllowanceFee(symbol);
	...
	private Double calculateGasForLRC(RawOrder order) {
		Double result;
		AccountBalance asset = balance.getAssetBySymbol("LRC");
