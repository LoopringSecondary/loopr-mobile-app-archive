...
/**
	...
	public static double Mrpi(double p,double r,int n){
		...
		return (new BigDecimal(p)).multiply(mr).multiply(aprPow).divide(aprPow.subtract(new BigDecimal(1)),2, BigDecimal.ROUND_DOWN).doubleValue();
	}
	...
	public static double getTotalInterest(double account,double apr,int period){
		...
		//总共需要还款金额
		double totalRemain= BigDecimal.valueOf(moneyPerMonth).multiply(BigDecimal.valueOf(period)).setScale(6, BigDecimal.ROUND_FLOOR).doubleValue();
		//每期还款后剩余金额
		...
		for(int i=0;i<period;i++){
			if(period - i>1){
				//计算每月需要支付的利息
				interest=format4(new BigDecimal(remain).multiply(new BigDecimal(apr)).divide(new BigDecimal(12),6,BigDecimal.ROUND_DOWN).doubleValue());
				//用于计算利息的剩余金额
