...
public protocol SwipeTabViewDataSource {
    func numberOfItems(in swipeTabView: SwipeTabView) -> Int
    func swipeTabView(_ swipeTabView: SwipeTabView, titleForItemAt index: Int) -> String?
}

open class SwipeTabView: UIScrollView {
    
    open var dataSource: SwipeTabViewDataSource!
    
    var itemViews: [SwipeTabItemView] = []
    ...
    fileprivate var options: SwipeViewOptions.SwipeTabView = SwipeViewOptions.SwipeTabView()
    
    private var leftMarginConstraint: NSLayoutConstraint = .init()
    private var widthConstraint: NSLayoutConstraint = .init()
    
    public init(frame: CGRect, options: SwipeViewOptions.SwipeTabView? = nil) {
    ...
    fileprivate func setupTabItemViews() {
        ...
        for index in 0..<itemCount {
            let tabItemView = SwipeTabItemView(frame: CGRect(x: xPosition, y: 0, width: options.itemView.width, height: containerView.frame.size.height))
            ...
            if let title = dataSource.swipeTabView(self, titleForItemAt: index) {
...
extension SwipeTabView {
    ...
    fileprivate func setupUnderlineView() {
        ...
        case .underline:
            ...
            underlineView.backgroundColor = UIColor.theme
...
extension SwipeTabView {
    ...
    var previousItem: SwipeTabItemView {
        if currentIndex > 0 {
            return itemViews[currentIndex - 1]
        }
        return itemViews[currentIndex]
