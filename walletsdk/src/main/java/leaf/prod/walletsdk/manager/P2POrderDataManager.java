...
public class P2POrderDataManager extends OrderDataManager {
	...
	private Boolean validate() {
		...
		if (orders.length >= 2) {
			...
			if (makerPrivateKey != null && !makerPrivateKey.isEmpty() &&
					taker.getParams()
							.getDualAuthPrivateKey() != null && maker.getHash() != null && taker.getHash() != null) {
				result = true;
			}
		}
		return result;
	...
	private void checkBalanceEnough(RawOrder order) {
		if (isTaker) {
			Double tokensBalance = token.getDoubleFromWei(order.getTokenS(), token.getTokenByProtocol(order.getTokenS())
					.getDecimals());
